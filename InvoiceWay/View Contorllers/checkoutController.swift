...
class checkoutController: UIViewController {
   ...
   var name = ""
    ...
    @IBAction func shipToCostPressed(_ sender: UIButton) {
        
        let defaults = UserDefaults.standard
        let token = defaults.string(forKey: "iboNum")
        
        ref.child("users/\(String(describing: token!))/customers/\(name)/address").setValue("\(email!.text!)")
...
let vc = storyboard!.instantiateViewController(withIdentifier: "shipping") as! shippingInfoController
    ...
    @IBAction func payWithCash(_ sender: UIButton) {
        print("clicked")
        let defaults = UserDefaults.standard
        let token = defaults.string(forKey: "iboNum")
